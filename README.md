...
<h1 align="center">Gald3r for SubQ</h1>

<p align="center">
  The <strong>SubQ edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside SubQ</strong>. Part of the Gald3r framework
...
This is the **SubQ distribution** of Gald3r. The [`subq/`](./subq/) folder is a
complete, ready-to-deploy gald3r setup tuned for SubQ -- drop its contents into your
...
cp -r gald3r_platform_subq/subq/. /path/to/your/project/
```

Then open the project in SubQ and type `@g-status` to confirm gald3r loaded.

> The `subq/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
